...
impl Node {
    ...
    fn process_find_node(&self, payload: FindNodePayload) {
        let _k_closest_nodes = self.routing_table.find_k_closest(payload.id, K as usize);

        // TODO: respond with KNODES.
    }

    fn process_k_nodes() {
        // TODO: insert the nodes into the peer list, then initiate connections for the relevant
        // nodes.
    }
    ...
    fn process_chunk(&self, payload: ChunkPayload) {
        ...
        let _selected_peers = self.routing_table.select_broadcast_peers(payload.h);

        // TODO: propagate the CHUNK message to each next_addr.
    }

    // TODO: implement disconnects & bucket eviction logic
}
